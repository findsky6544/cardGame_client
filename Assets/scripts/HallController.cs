...
public class HallController : MonoBehaviour {
    ...
    public void onCreateRoomFailed(ushort retcode)
    {
        Debug.LogFormat("创建房间失败，原因:{0}", KBEngineApp.app.serverErr(retcode));
        MessagePanelController.openMessagePanel(canvas, "创建房间失败，原因:" + KBEngineApp.app.serverErr(retcode));
    }
    ...
    public void onReqJoinRoomSuccess(ulong roomKey)
    {
        //int ret = Init.Voiceengine.JoinTeamRoom(roomKey.ToString(), 15000);
    ...
    void Start ()
    {
        ...
        KBEngine.Event.registerOut("onCreateRoomFailed", this, "onCreateRoomFailed");
